...
class ChristmasDayDiscountTest {
    ...
    fun `12월이 아닌 경우 할인 금액은 0원이다`() {
        val discount = ChristmasDayDiscount(date = Date(year = 2023, month = 11, day = 25))
    ...
    fun `12월이 25일이 지난 경우 할인 금액은 0원이다`() {
        val discount = ChristmasDayDiscount(date = Date(year = 2023, month = 12, day = 26))
        val amount = discount.calculate()
        assert(amount == 0)
    }
    ...
    fun `12월이 1일에서 25일 사이는 할인을 받는다`(day: Int, expectedAmount: Int) {
        val discount = ChristmasDayDiscount(date = Date(year = 2023, month = 12, day = day))
